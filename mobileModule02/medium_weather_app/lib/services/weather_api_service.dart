...
class WeatherApiService {
  static Future<List<Location>> fetchLocations(String query) async {
    final String uri = TextConstants.citiesUri.replaceFirst('<substring>', query);
  ...
  static Future<CurrentWeatherData> fetchCurrentWeather(double latitude, double longitude) async {
  ...
  static Future<HourlyWeatherData> fetchTodayWeather(double latitude, double longitude) async {
    ...
    if (response.statusCode == 200) {
      ...
      return HourlyWeatherData(
        ...
        hourlyTime: List<DateTime>.from(hourly['time'].map((time) => DateTime.parse(time))),
        hourlyTemperature: List<double>.from(hourly['temperature_2m']),
        hourlyWeatherCodeDescription: List<int>.from(hourly['weather_code']),
  ...
  static Future<WeeklyWeatherData> fetchWeeklyWeather(double latitude, double longitude) async {
    ...
    if (response.statusCode == 200) {
      ...
      return WeeklyWeatherData(
        ...
        dailyTime: List<DateTime>.from(daily['time'].map((time) => DateTime.parse(time))),
  ...
  Future<WeatherData> fetchWeatherData(WeatherType type, Location location) async {
    try {
      switch (type) {
        case WeatherType.tab1:
          final currentWeather = await fetchCurrentWeather(location.latitude, location.longitude);
          return currentWeather;
        case WeatherType.tab2:
          final todayWeather = await fetchTodayWeather(location.latitude, location.longitude);
          return todayWeather;
        case WeatherType.tab3:
          final weeklyWeather = await fetchWeeklyWeather(location.latitude, location.longitude);
